...
- **Recommender Loop**: The Recommender operates in a continuous loop, running at intervals specified by the `--recommender-interval` flag. In each iteration, it performs the following steps, as defined in the [RunOnce function](https://github.com/kubernetes/autoscaler/blob/vertical-pod-autoscaler-1.3.0/vertical-pod-autoscaler/pkg/recommender/main.go#L314):
...
CPU `Usage` and CPU `Requests` are converted into CPU cores, meaning that 233 milicores of `Usage` are converted to 0.233 CPU cores. Next, we determine the CPU sample's weight. If the `Request` field contains a zero value, the weight is set to 0.1 by default. Otherwise, the weight is equal to the current request in CPU cores. The next step is to multiply the weight by the value returned from the decaying factor function (`decayFactor`). The decaying factor is used to reduce the sample's weight ("importance") by half for each `halfLife` period, meaning that newer samples receive a higher weight than older ones. By default, the `halfLife` is set to 24 hours. In our example, the decaying factor is 161.793, which is then multiplied by 0.1 to give the final weight of 16.1793. Below is an example of how the decaying factor function's return value increases as the `MeasureStart` timestamp increases:
...
Now that we have a usage value (0.233), a final weight (16.1793 = 161.793 * 0.1), and a timestamp (2025-02-01T08:06:44Z), we are ready to aggregate this CPU sample. The Recommender will then find the Kubernetes deployment's CPU histogram to which the sample belongs. Let's assume this is the CPU histogram before aggregation:
...
Next, we determine the bucket's index using the [FindBucket](https://github.com/kubernetes/autoscaler/blob/vertical-pod-autoscaler-1.3.0/vertical-pod-autoscaler/pkg/recommender/util/histogram_options.go#L120) function. CPU histograms use an exponential bucketing scheme, with the smallest bucket size being 0.01 core and a maximum of 1000.0 cores. With the above-mentioned parameters, such as `firstBucketSize`, `ratio` and `numBuckets`, the first bucket starts at 0 with index 0, the second at 0.01, and the third at 0.020499. The start of a bucket with a given index is calculated using the `GetBucketStart` function.

In this case, with a usage of 0.233 CPU cores, the bucket index will be 15. Once we identify the bucket index, we add the new weight (16.1793) to it. The updated `bucketWeight[15]` will be 16.1793:
...
1. If the new memory sample is larger than the current peak and its timestamp is earlier than the end of the current aggregation interval (`WindowEnd`), the Recommender follows these steps:
    1. Subtract the old peak memory sample’s weight from the Memory Histogram using [this function](https://github.com/kubernetes/autoscaler/blob/vertical-pod-autoscaler-1.3.0/vertical-pod-autoscaler/pkg/recommender/model/aggregate_container_state.go#L198). The function uses the old peak value in bytes, the weight (calculated as 1.0 * `decayFactor(WindowEnd)`), and the timestamp (which is the container’s `WindowEnd`).
    2. Add the new peak’s weight to the Memory histogram’s appropriate bucket based on the new peak value. This is done using the [AddSample](https://github.com/kubernetes/autoscaler/blob/vertical-pod-autoscaler-1.3.0/vertical-pod-autoscaler/pkg/recommender/util/decaying_histogram.go#L65) function. The bucket index for the new weight is returned by the `FindBucket` function. The arguments for the `AddSample` function are:
...
Memory histograms use an exponential bucketing scheme (just like CPU histograms), with the smallest bucket size being 10MB and the maximum size being 1TB. The bucket with index 0 starts at 0, the next starts at 10 MB, followed by 20.49 MB, and then 31.5249 MB.
...
VPA is a powerful tool for automatically rightsizing workloads and reducing resource _slack_. However, it may not be suitable for everyone. For example, if your applications cannot tolerate disruptions, the VPA might not be the right fit for you.